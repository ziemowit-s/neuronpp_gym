...
class EbnerOlfactoryAgent(Agent):
    ...
    def _build_network(self, input_cell_num, input_size, output_cell_num):
        ...
        # INHIBITORY NFB
        for i in range(4):
            self._make_inhibitory_cells(counter=i, sources=self.hidden_pop.cells[i:i + 3], netcon_weight=0.1)
    ...
    def _make_modulatory_population(self, name, cell_num, source=None):
        ...
        syns = pop.connect(source=source, syn_num_per_source=1,
                           delay=1, random_weight_mean=1.0, netcon_weight=0.01, neuromodulatory_weight=0.1, rule='all')
